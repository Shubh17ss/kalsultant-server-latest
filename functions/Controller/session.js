...
const storeProposedSession = async (req, res) => {
    try {
        await connectDB();
        let { firstName, lastName, email, dob, tob, gender, pob, date, slot, proposed_slot, contactNumber } = req.body;
        let name = firstName + ' ' + lastName;
        let proposedSlot = proposed_slot;
        const session_date = date;
...
const getBookedSLots = (req, res) => {
    ...
    res.status(200).json({ message: "This controller has not been built yet" });
    return;
}
